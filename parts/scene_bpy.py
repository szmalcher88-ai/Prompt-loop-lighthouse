...
def build(seed=0, **params):
    """Powierzchnia kontraktu części (parts/README.md) dla check_parts: assembler
    odpowiada w świecie OBJ za DROBIAZGI, więc w trybie stdlib zwraca właśnie ich
    grupy (rdzeń buduje bpy przez core_placements()). Pipeline hybrydy NIE woła
    build() — używa core_placements()/write_drobiazgi()."""
    return _drobiazgi_groups()
